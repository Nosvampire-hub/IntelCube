...
public class MultiplayerGameBase : MonoBehaviour
{
	...
	void Update()
    {
        if (playerSpawned)
        {
            if (!baseSpawned)
            {
                ...
                playerActor = GameObject.Find("Me");
                playerActor.GetComponent<MultiPlayerController>().KillUnKill();
            }
    ...
    {
		GameObject player = GameObject.Find("Me");
		player.GetComponent<MultiPlayerController>().KillUnKill();
